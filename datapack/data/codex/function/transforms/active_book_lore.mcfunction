...
#data modify storage codex:internal root.transforms."codex:active_book_lore".out \
  append value ""
data modify storage codex:internal root.transforms."codex:active_book_lore".out \
  append value {translate: "codex.book.active.1", fallback: "Scroll on this item to see keywords,", with: [{translate: "codex.bracket", fallback: "[%s]", with: [{keybind: "key.use", color: "yellow"}]}], color: "white", italic: false}
data modify storage codex:internal root.transforms."codex:active_book_lore".out \
